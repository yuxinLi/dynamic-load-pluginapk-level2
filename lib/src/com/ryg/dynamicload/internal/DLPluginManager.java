...
public class DLPluginManager {
    ...
    public int startPluginActivityForResult(Context context, DLIntent dlIntent, int requestCode) {
        ...
        //拿到保存的 DLPluginPackage ,包含插件的 packageName、defaultActivity、classLoader、assetManager、resources、packageInfo
        DLPluginPackage pluginPackage = mPackagesHolder.get(packageName);
        if (pluginPackage == null) {
            return START_RESULT_NO_PKG;
        }
        // 拿到插件的 launchActivty = defaultActivity
        final String className = getPluginActivityFullPath(dlIntent, pluginPackage);
        // 加载launchActivty 的 clazz, 用 pluginPackage的 classLoader
        ...
        // plugin activity.
        // 获取写好的的代理Activity，插件Activity由代理Activity启动
