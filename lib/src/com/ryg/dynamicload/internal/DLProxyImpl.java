...
public class DLProxyImpl {
    ...
    protected void launchTargetActivity() {
        try {
            ...
            // 把插件 Activity 强转成的接口 DLPlugin 和 mPluginManager 保存到 代理Activity
            ((DLAttachable) mProxyActivity).attach(mPluginActivity, mPluginManager);
            Log.d(TAG, "instance = " + instance);
            // attach the proxy activity and plugin package to the mPluginActivity
            // 把代理Activity 和 mPluginManager 保存到 插件Activity
            mPluginActivity.attach(mProxyActivity, mPluginPackage);

            Bundle bundle = new Bundle();
            bundle.putInt(DLConstants.FROM, DLConstants.FROM_EXTERNAL);
            // 调用 插件Activity的 oncreate，里面会调用 代理Activity 的setContentView ，把插件的layout 加载到 代理Activity
